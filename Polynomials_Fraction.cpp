    ...
    public:
        ...
        int makeSimple()
        {
            for (int i=0;i<(pLen-1);i++)
            {
                ...
                for (int j=i+1;j<pLen;j++)
                {
                    ...
                    {
                        term[i].a=term[i].a*term[j].b+term[i].b*term[j].a;
                        term[i].b*=term[j].b;
                        if (term[i].a>=0)
                        {
                            term[i].isPlus=1;
        ...
        void print()
        {
            ...
            for (int i=0;i<aLen;i++)
            {
                if (term[i].a==0)
                    continue;
                int gcd=fraction(abs(term[i].a),term[i].b);
                term[i].a/=gcd;
                term[i].b/=gcd;
                cout<<term[i].mark<<'('<<abs(term[i].a)<<'/'<<abs(term[i].b)<<")x^"<<term[i].n;
                flag=1;
            }
            if (flag==0)
               cout<<"+0x^0";
...
Polynomial operator+(Polynomial poly1,Polynomial poly2)
{
    ...
    for(int i=0;i<poly2.aLen;i++)
    {
        ...
        for(int j=0;j<tmp.pLen;j++)
        {
            ...
            {
                tmp.term[j].a=tmp.term[j].a*poly2.term[i].b+tmp.term[j].b*poly2.term[i].a;
                tmp.term[j].b*=poly2.term[i].b;
                flag=1;
                if (tmp.term[j].a>=0 )
                {
                    tmp.term[j].isPlus=1;
...
Polynomial operator-(Polynomial poly1,Polynomial poly2)
{
    ...
    for(int i=0;i<poly2.aLen;i++)
    {
        ...
        for(int j=0;j<tmp.pLen;j++)
        {
            if(tmp.term[j].n==poly2.term[i].n)
            {
                tmp.term[j].a=tmp.term[j].a*poly2.term[i].b-poly2.term[i].a*tmp.term[j].b;
                tmp.term[j].b*=poly2.term[i].b;
                if (tmp.term[j].a>=0 )
                {
                    tmp.term[j].isPlus=1;
...
Polynomial operator*(Polynomial poly1,Polynomial poly2)
{
    ...
    for (int i=0;i<poly1.aLen;i++)
    {
        for (int j=0;j<poly2.aLen;j++)
        {
            ...
            tmp.term[tmp.aLen].b=poly1.term[i].b*poly2.term[j].b;
...
int main()
{
    Term term1[N];
    Term term2[N];
    cout<<"Enter two polynomials in the next two lines:\t\t\t(like this;+(0/3)x^3-(6/1)x^4-(22/4)x^0)"<<endl;
    ...
    for (int i=0;i<N;i++)
    {
        ...
        if(term1[i].isPlus==0)
            term1[i].a*=-1;
        getchar();getchar();getchar();       //passing ), x and ^
        cin>>term1[i].n;
        P1++;
    }
